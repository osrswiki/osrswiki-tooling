...
(function() {
    ...
    window.osrsInstallNativeCalcSlot = function (opts) {
        ...
        style.textContent = [
            ...
            'html.osrs-native-calc-slot-active div.jcConfig,',
    ...
    function osrsJcConfigSelector() {
        var api = window.osrsIndocCalc;
        return (api && api.JC_CONFIG_SELECTOR) || 'pre.jcConfig, div.jcConfig';
    }
    ...
    function osrsIndocShowPicker(label, options, currentValue, onPick, searchable) {
    ...
    function osrsIndocBindForm(slot, definition, values) {
        ...
        function currentValues() {
            ...
            slot.querySelectorAll('[data-osrs-indoc-name], input[name], select[name]').forEach(function (node) {
                ...
                if (node.type === 'checkbox') {
                    var checkModel = definition.inputs.filter(function (input) { return input.name === name; })[0];
                    if (checkModel && checkModel.type === 'check' && checkModel.options && checkModel.options.length >= 2) {
                        next[name] = node.checked ? checkModel.options[0] : checkModel.options[1];
                    } else {
            ...
            definition.inputs.forEach(function (input) {
                if (input.type !== 'togglebuttongroup') return;
                var pressed = [];
                slot.querySelectorAll(
                    '[data-osrs-indoc-chip="togglebuttongroup"][data-osrs-indoc-name="' + input.name + '"][aria-pressed="true"]'
                ).forEach(function (chip) {
                    var option = chip.getAttribute('data-osrs-indoc-option');
                    if (option) pressed.push(option);
                });
                next[input.name] = pressed.join(',');
            });
            return next;
        }
        ...
        function clearLookupOutput() {
            setBanner('');
            setStatus('');
            if (typeof window.osrsNativeCalcSetResult === 'function') {
                window.osrsNativeCalcSetResult(definition.ui.resultId, '');
            }
            osrsClearCalculatorPublishEcho();
            osrsHideEmptyCalculatorOutputs();
        }

        function showLookupError(player) {
            var err = api.missingPlayerMessage(player);
            if (typeof window.osrsNativeCalcSetResult === 'function') {
                window.osrsNativeCalcSetResult(definition.ui.resultId, api.lookupErrorHtml(err));
            }
            setBanner('');
            setStatus('');
        ...
        function bind() {
            ...
            form.addEventListener('click', function (event) {
                ...
                if (chip) {
                    ...
                    var option = chip.getAttribute('data-osrs-indoc-option');
                    var chipKind = chip.getAttribute('data-osrs-indoc-chip') || 'buttonselect';
                    if (chipKind === 'togglebuttongroup') {
                        var selected = String(values[chipName] || '').split(',').map(function (s) {
                            return s.trim();
                        }).filter(Boolean);
                        var idx = selected.indexOf(option);
                        if (idx >= 0) selected.splice(idx, 1);
                        else selected.push(option);
                        values[chipName] = selected.join(',');
                    } else {
                        values[chipName] = option;
                    }
                    rerender();
                    if (api.shouldAutosubmit(definition)) submit();
                    return;
                ...
                if (select) {
                    ...
                    osrsIndocShowPicker(
                        model.label,
                        model.options,
                        values[selectName] || model.defaultValue,
                        function (picked) {
            ...
            form.addEventListener('change', function (event) {
                ...
                if (target.type === 'checkbox') {
                    var checkModel = definition.inputs.filter(function (input) { return input.name === target.name; })[0];
                    if (checkModel && checkModel.type === 'check' && checkModel.options && checkModel.options.length >= 2) {
                        values[target.name] = target.checked ? checkModel.options[0] : checkModel.options[1];
                    } else {
...
function osrsPromoteCalculatorDescription(layout, formHost) {
        ...
        while (prev) {
            ...
            if (prev.querySelector && prev.querySelector('pre.jcConfig, div.jcConfig, .jcTable, .oo-ui-widget')) break;
            prev = prev.previousElementSibling;
        }
        if (!desc) {
            var root = document.querySelector('.mw-parser-output') || document.body;
            var pre = root.querySelector(osrsJcConfigSelector());
            if (pre) {
        ...
        function extractDropdownOptions(element) {
            ...
            if (dropdownRoot) {
                ...
                if (!options.length && label) {
                    var pre = document.querySelector(osrsJcConfigSelector());
                    if (pre) {
...
function parseCalculatorConfig(pre) {
    ...
    function findCalculatorTemplateBox(pre) {
        ...
        while (previous) {
            ...
            if (previous.matches('table, pre.jcConfig, div.jcConfig, h1, h2, h3')) {
                break;
            }
    ...
    function osrsReassertCalculatorThemeSheets() {
        if (!document.querySelector('.osrs-calculator-layout, pre.jcConfig, div.jcConfig')) {
            return;
        }
    ...
    function osrsInstallCalculatorThemeSkin() {
        var css = [
            ...
            'html body .osrs-calculator-layout .jsCalc-field-fixed>.oo-ui-fieldLayout-body>.oo-ui-fieldLayout-field>.oo-ui-labelElement-label:empty,html body .osrs-calculator-layout .osrs-calculator-result:empty,html body .osrs-calculator-layout [id$="Result"]:empty,html body .osrs-calculator-layout [id$="Results"]:empty,html body #osrs-calculator-status:empty{display:none!important;background:none!important;border:0!important;min-height:0!important;height:0!important;padding:0!important;margin:0!important}',
    ...
    function osrsHideEmptyCalculatorOutputs() {
        document.querySelectorAll('.osrs-calculator-layout [id$="Result"], .osrs-calculator-layout [id$="Results"], .osrs-calculator-result, #osrs-calculator-status').forEach(function (node) {
    ...
    function loadCalcCore(attempt) {
        ...
        if (!document.querySelector(osrsJcConfigSelector())) {
            return;
        }
    ...
    function osrsClearCalculatorPublishEcho() {
        var status = document.getElementById('osrs-calculator-status');
        if (status) status.textContent = '';
        if (document.documentElement) {
            document.documentElement.removeAttribute('data-osrs-calc-result');
        }
    }

    function osrsCalculatorResultSourceNode() {
        var nodes = document.querySelectorAll(
            '.osrs-calculator-result, [id$="Result"], [id$="Results"]'
        );
        var i, node, filled = null, fallback = null;
        for (i = 0; i < nodes.length; i++) {
            node = nodes[i];
            if (!node) continue;
            if (node.id === 'osrs-calculator-status') continue;
            if (node.id === 'osrs-indoc-calc-banner' || node.id === 'osrs-indoc-calc-status') continue;
    ...
    function osrsPublishCalculatorResult() {
        var node = osrsCalculatorResultSourceNode();
        var status = document.getElementById('osrs-calculator-status');
    ...
    function osrsWatchCalculatorResults() {
        ...
        document.querySelectorAll('[id$="Result"], [id$="Results"], [id$="Form"], #bodyContent').forEach(function(node) {
    ...
    function initialize() {
        ...
        osrsInstallCalculatorKeyboardGuards();
        document.querySelectorAll(osrsJcConfigSelector()).forEach(function(pre) {
